...
void Application3D::update(float deltaTime) {
	...
	if (input->isKeyDown(aie::INPUT_KEY_UP))
		camPosZ += 5.0f * deltaTime;

	if (input->isKeyDown(aie::INPUT_KEY_DOWN))
		camPosZ -= 5.0f * deltaTime;

	if (input->isKeyDown(aie::INPUT_KEY_LEFT))
		camPosX -= 5.0f * deltaTime;

	if (input->isKeyDown(aie::INPUT_KEY_RIGHT))
		camPosX += 5.0f * deltaTime;

	if (input->isKeyDown(aie::INPUT_KEY_LEFT_SHIFT))
		camPosY += 5.0f * deltaTime;

	if (input->isKeyDown(aie::INPUT_KEY_LEFT_CONTROL))
		camPosY -= 5.0f * deltaTime;

	// rotate camera
	m_viewMatrix = glm::lookAt(vec3(camPosX, camPosY, camPosZ),
							   vec3(0), vec3(0, 1, 0));

	std::cout << "Current Positions: " << (int)camPosX << "/" << (int)camPosY << "/" << (int)camPosZ << std::endl;
